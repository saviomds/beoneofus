      ...
      <div className="relative h-14 bg-gradient-to-br from-slate-700 to-slate-900 flex items-center px-5">
        {company.verified && (
          <div className="absolute top-2.5 right-3 flex items-center gap-1 text-[9px] font-black text-blue-300 bg-blue-900/40 border border-blue-700/50 px-2 py-0.5 rounded-full uppercase tracking-widest">
            <CheckCircle2 size={9} /> {t('companies.verified')}
          </div>
        )}
        {company.hiring && (
      ...
      <div className="px-5 pb-5">
        <div className="flex items-end gap-3 -mt-5 mb-3">
          <div className="w-12 h-12 rounded-xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 flex items-center justify-center shadow-sm shrink-0">
            {company.logo_url ? (
              <Image src={company.logo_url} alt="" width={40} height={40} unoptimized className="w-10 h-10 object-contain rounded-lg" />
            ) : (
          ...
          {isOwner && <span className="text-[9px] font-black text-amber-600 dark:text-amber-400 uppercase tracking-widest bg-amber-50 dark:bg-amber-900/20 px-2 py-0.5 rounded-md border border-amber-200 dark:border-amber-800/50 shrink-0 mb-1">{t('companies.owner')}</span>}
        </div>
  ...
  return (
    <div className="space-y-5 animate-in fade-in slide-in-from-bottom-2 duration-300">
      ...
      <div className="flex flex-wrap gap-2">
        {company.verified && <span className="inline-flex items-center gap-1 text-[10px] font-black text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800/50 px-2.5 py-1 rounded-lg uppercase tracking-widest"><CheckCircle2 size={10} /> {t('companies.verified')}</span>}
        {company.hiring && <span className="inline-flex items-center gap-1.5 text-[10px] font-black text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800/50 px-2.5 py-1 rounded-lg uppercase tracking-widest"><span className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse" /> {t('companies.activelyHiring')}</span>}
        ...
        {company.size && <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2.5 py-1 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-600 dark:text-gray-400"><Users size={10} /> {t('companies.employees', { size: company.size })}</span>}
        {company.founded_year && <span className="text-[10px] font-bold px-2.5 py-1 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-600 dark:text-gray-400">{t('companies.established', { year: company.founded_year })}</span>}
      ...
      {company.description && (
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-5">
          <p className="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-2">{t('companies.about')}</p>
      ...
      {company.tech_stack?.length > 0 && (
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-5">
          <p className="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-3 flex items-center gap-1.5"><Code2 size={11} /> {t('companies.techStack')}</p>
          <div className="flex flex-wrap gap-2">
      ...
      {company.culture && (
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-5">
          <p className="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-2">{t('companies.culture')}</p>
      ...
      {company.hiring && company.website && (
        <a href={company.website} target="_blank" rel="noopener noreferrer" className="block w-full py-4 bg-blue-600 hover:bg-blue-500 text-white font-black rounded-2xl transition-all text-center text-sm shadow-lg shadow-blue-500/20 flex items-center justify-center gap-2">
          <Briefcase size={16} /> {t('companies.viewOpenRoles')} <ExternalLink size={14} />
        </a>
      )}
    </div>
  );
...
function CreateCompanyModal({ token, onClose, onCreated }) {
  ...
  const handleSubmit = async () => {
    if (!form.name) { setError(t('companies.errorNameRequired')); return; }
    setSaving(true);
    setError('');
    try {
    ...
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-900 rounded-3xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl border border-gray-200 dark:border-gray-700 animate-in slide-in-from-bottom-4 sm:zoom-in-95 duration-300">
        <div className="sticky top-0 bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 px-5 py-4 flex items-center justify-between rounded-t-3xl z-10">
          <h3 className="font-black text-gray-900 dark:text-gray-100">{t('companies.addProfile')}</h3>
        ...
        <div className="p-5 space-y-4">
          {[
            { key: 'name', label: t('companies.nameLabel'), placeholder: t('companies.namePlaceholder') },
            { key: 'website', label: t('companies.websiteLabel'), placeholder: t('companies.websitePlaceholder') },
            { key: 'location', label: t('companies.locationLabel'), placeholder: t('companies.locationPlaceholder') },
            { key: 'founded_year', label: t('companies.foundedLabel'), placeholder: t('companies.foundedPlaceholder') },
          ...
          <div>
            <label className="text-[10px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest block mb-1.5">{t('companies.aboutLabel')}</label>
            <textarea value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} rows={3} placeholder={t('companies.aboutPlaceholder')} className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-3.5 py-2.5 text-sm text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 resize-none transition-all" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            {[
              { key: 'industry', label: t('companies.industryLabel'), options: INDUSTRIES },
              { key: 'size', label: t('companies.sizeLabel'), options: COMPANY_SIZES },
              { key: 'remote_policy', label: t('companies.remoteLabel'), options: REMOTE_POLICIES },
              ...
              <div key={key} className={key === 'industry' ? 'col-span-2' : ''}>
                ...
                <select value={form[key]} onChange={e => setForm(p => ({ ...p, [key]: e.target.value }))} className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-3.5 py-2.5 text-sm text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500/20 transition-all">
                  <option value="">{t('companies.selectOption')}</option>
          ...
          <div>
            <label className="text-[10px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest block mb-1.5">{t('companies.culture')}</label>
            <textarea value={form.culture} onChange={e => setForm(p => ({ ...p, culture: e.target.value }))} rows={2} placeholder={t('companies.culturePlaceholder')} className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-3.5 py-2.5 text-sm text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 resize-none transition-all" />
          </div>
          <div>
            <label className="text-[10px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest block mb-1.5">{t('companies.techStack')}</label>
            ...
            <div className="flex gap-2">
              <input value={techInput} onChange={e => setTechInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && techInput.trim()) { addTech(techInput.trim()); setTechInput(''); }}} placeholder={t('companies.customTechPlaceholder')} className="flex-1 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2 text-xs text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 transition-all" />
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            <span className="text-sm font-bold text-gray-700 dark:text-gray-300">{t('companies.currentlyHiring')}</span>
          </label>
          ...
          <button onClick={handleSubmit} disabled={saving} className="w-full py-3.5 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white font-black rounded-xl transition-all flex items-center justify-center gap-2 text-sm active:scale-95">
            ...
            {saving ? t('companies.creating') : t('companies.addProfile')}
          </button>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="max-w-2xl mx-auto w-full space-y-5">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-black text-gray-900 dark:text-gray-100 tracking-tighter">{t('companies.title')}</h1>
          <p className="text-gray-500 dark:text-gray-400 text-sm mt-0.5 font-medium">{t('companies.subtitle')}</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          {session && <button onClick={() => setShowCreate(true)} className="flex items-center gap-1.5 bg-blue-600 hover:bg-blue-500 text-white text-sm font-black px-4 py-2.5 rounded-xl transition-all active:scale-95 shadow-sm shadow-blue-500/20"><Plus size={15} /> {t('companies.addCompany')}</button>}
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <div className="relative flex-1 min-w-40">
          ...
          <input type="text" value={search} onChange={e => setSearch(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') fetchCompanies(hiringOnly, e.target.value); }} placeholder={t('companies.searchPlaceholder')} className="w-full pl-8 pr-3 py-2.5 text-sm bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all" />
        </div>
        <button onClick={() => setHiringOnly(v => !v)} className={`flex items-center gap-1.5 text-[11px] font-black px-4 py-2.5 rounded-xl transition-all whitespace-nowrap ${hiringOnly ? 'bg-emerald-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 hover:text-emerald-600 dark:hover:text-emerald-400'}`}>
          <span className="w-1.5 h-1.5 bg-current rounded-full" /> {t('companies.hiringNow')}
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          ...
          <h3 className="text-base font-bold text-gray-900 dark:text-gray-100">{t('companies.emptyTitle')}</h3>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{t('companies.emptyDesc')}</p>
          {session && <button onClick={() => setShowCreate(true)} className="mt-4 bg-blue-600 hover:bg-blue-500 text-white font-bold py-2.5 px-6 rounded-xl transition-all text-sm">{t('companies.addCompany')}</button>}
        </div>
      )}
    </div>
  );
