    ...
    <div className="space-y-5 animate-in fade-in slide-in-from-bottom-2 duration-300">
      ...
      {job.description && (
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-5">
          <p className="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-2">{t('freelance.description')}</p>
      ...
      {job.skills?.length > 0 && (
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-4">
          <p className="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-2 flex items-center gap-1.5"><Tag size={11} /> {t('freelance.required_skills')}</p>
        ...
        <div>
          {applied ? (
            <div className="flex items-center gap-2 p-4 bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-200 dark:border-emerald-800/50 rounded-2xl">
              ...
              <div>
                <p className="text-sm font-bold text-emerald-700 dark:text-emerald-300">{t('freelance.application_sent')}</p>
                <p className="text-xs text-emerald-600/70 dark:text-emerald-400/70">{t('freelance.application_sent_desc')}</p>
              </div>
            </div>
          ) : showApply ? (
            <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-5 space-y-4">
              <p className="text-sm font-black text-gray-900 dark:text-gray-100">{t('freelance.apply_for_role')}</p>
              <textarea value={coverNote} onChange={e => setCoverNote(e.target.value)} placeholder={t('freelance.cover_note_ph')} rows={4} className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 text-sm text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 resize-none transition-all" />
              <input value={portfolioUrl} onChange={e => setPortfolioUrl(e.target.value)} placeholder={t('freelance.portfolio_url_ph')} className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-2.5 text-sm text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all" />
              <input value={rate} onChange={e => setRate(e.target.value)} placeholder={t('freelance.rate_ph')} className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-2.5 text-sm text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all" />
              ...
              <div className="flex gap-2">
                <button onClick={() => setShowApply(false)} className="flex-1 py-2.5 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 text-sm font-bold rounded-xl transition-all">{t('freelance.cancel')}</button>
            ...
            <button onClick={() => session ? setShowApply(true) : window.location.href = '/auth'} className="w-full py-4 bg-blue-600 hover:bg-blue-500 text-white font-black rounded-2xl transition-all flex items-center justify-center gap-2 text-sm active:scale-95 shadow-lg shadow-blue-500/20">
              <Send size={16} /> {t('freelance.apply_now')}
            </button>
          )}
        </div>
      )}
      ...
      {job.apply_url && !applied && (
        <a href={job.apply_url} target="_blank" rel="noopener noreferrer" className="block w-full py-4 bg-blue-600 hover:bg-blue-500 text-white font-black rounded-2xl transition-all text-center text-sm shadow-lg shadow-blue-500/20">
          {t('freelance.apply_external')} <ExternalLink size={14} className="inline ml-1.5" />
        </a>
      )}
    </div>
  );
...
function PostJobModal({ token, onClose, onPosted }) {
  ...
  const handleSubmit = async () => {
    if (!form.title || !form.description) { setError(t('freelance.error_required')); return; }
    setSaving(true);
    setError('');
    try {
    ...
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-900 rounded-3xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl border border-gray-200 dark:border-gray-700 animate-in slide-in-from-bottom-4 sm:zoom-in-95 duration-300">
        <div className="sticky top-0 bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 px-5 py-4 flex items-center justify-between rounded-t-3xl z-10">
          <h3 className="font-black text-gray-900 dark:text-gray-100">{t('freelance.modal_title')}</h3>
        ...
        <div className="p-5 space-y-4">
          {[
            { key: 'title', label: t('freelance.job_title'), type: 'text', placeholder: t('freelance.job_title_ph') },
            { key: 'company', label: t('freelance.company'), type: 'text', placeholder: t('freelance.company_ph') },
          ...
          <div>
            <label className="text-[10px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest block mb-1.5">{t('freelance.description_star')}</label>
            <textarea value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} placeholder={t('freelance.description_ph')} rows={4} className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-3.5 py-2.5 text-sm text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 resize-none transition-all" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-[10px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest block mb-1.5">{t('freelance.region')}</label>
            ...
            <div>
              <label className="text-[10px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest block mb-1.5">{t('freelance.type')}</label>
            ...
            <div>
              <label className="text-[10px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest block mb-1.5">{t('freelance.budget')}</label>
              <input type="text" value={form.budget} onChange={e => setForm(p => ({ ...p, budget: e.target.value }))} placeholder={t('freelance.budget_ph')} className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-3.5 py-2.5 text-sm text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 transition-all" />
            </div>
            <div>
              <label className="text-[10px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest block mb-1.5">{t('freelance.duration')}</label>
              <input type="text" value={form.duration} onChange={e => setForm(p => ({ ...p, duration: e.target.value }))} placeholder={t('freelance.duration_ph')} className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-3.5 py-2.5 text-sm text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 transition-all" />
            </div>
          </div>
          <div>
            <label className="text-[10px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest block mb-1.5">{t('freelance.required_skills')}</label>
            ...
            <div className="flex gap-2">
              <input value={skillInput} onChange={e => setSkillInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && skillInput.trim()) { addSkill(skillInput.trim()); setSkillInput(''); }}} placeholder={t('freelance.add_skill_ph')} className="flex-1 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2 text-xs text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 transition-all" />
          ...
          <div>
            <label className="text-[10px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest block mb-1.5">{t('freelance.external_url')} <span className="font-medium normal-case text-gray-400">{t('freelance.external_url_note')}</span></label>
          ...
          <button onClick={handleSubmit} disabled={saving} className="w-full py-3.5 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white font-black rounded-xl transition-all flex items-center justify-center gap-2 text-sm active:scale-95">
            ...
            {saving ? t('freelance.posting') : t('freelance.post_job')}
          </button>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="max-w-2xl mx-auto w-full space-y-5">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-black text-gray-900 dark:text-gray-100 tracking-tighter">{t('freelance.title')}</h1>
          <p className="text-gray-500 dark:text-gray-400 text-sm mt-0.5 font-medium">{t('freelance.subtitle')}</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          {session && (
            <button onClick={() => setShowPost(true)} className="flex items-center gap-1.5 bg-blue-600 hover:bg-blue-500 text-white text-sm font-black px-4 py-2.5 rounded-xl transition-all active:scale-95 shadow-sm shadow-blue-500/20">
              <Plus size={15} /> {t('freelance.post_job')}
            </button>
          )}
        </div>
      </div>
        ...
        {CATEGORIES.map(c => {
          ...
          return (
            <button key={c} onClick={() => setCategory(c)} className={`shrink-0 flex items-center gap-1.5 text-[11px] font-black px-3 py-2 rounded-xl transition-all ${category === c ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700'}`}>
              ...
              {c === 'all' ? t('freelance.all_regions') : c}
            </button>
          );
        })}
      </div>
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar">
        {JOB_TYPES.map(jt => (
          <button key={jt} onClick={() => setType(jt)} className={`shrink-0 text-[11px] font-black px-3 py-2 rounded-xl transition-all capitalize ${type === jt ? 'bg-gray-900 dark:bg-white text-white dark:text-gray-900' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700'}`}>
            {jt === 'all' ? t('freelance.all_types') : jt}
          </button>
        ))}
        <div className="relative shrink-0 ml-auto">
          <Search size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
          <input type="text" value={search} onChange={e => setSearch(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') fetchJobs(category, type, e.target.value); }} placeholder={t('freelance.search_ph')} className="pl-8 pr-3 py-2 text-xs bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all w-36" />
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          ...
          <h3 className="text-base font-bold text-gray-900 dark:text-gray-100">{t('freelance.no_jobs')}</h3>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{t('freelance.no_jobs_desc')}</p>
          {session && <button onClick={() => setShowPost(true)} className="mt-4 bg-blue-600 hover:bg-blue-500 text-white font-bold py-2.5 px-6 rounded-xl transition-all text-sm">{t('freelance.post_a_job_cta')}</button>}
        </div>
      )}
    </div>
  );
