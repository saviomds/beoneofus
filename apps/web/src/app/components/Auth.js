      ...
      <div className="space-y-3">
        ...
        >
          ...
          <div className="min-w-0">
            ...
            <p className="text-[13px] text-gray-500 dark:text-gray-400 mt-0.5 leading-snug">One profile, every opportunity — jobs, a verified network, a marketplace, and projects.</p>
          </div>
        </button>
...
// Primary submit button. Module-scope so it keeps a stable component identity.
function PrimaryBtn({ label, loading }) {
  return (
    <button
      type="submit"
      disabled={loading}
...
function OAuthBlock({ handleOAuth, loading }) {
  return (
...
export default function AuthForm() {
  ...
  useEffect(() => {
    ...
    // Surface an error banner from the URL param on mount. Reading external
  ...
  useEffect(() => {
    ...
    // Pre-fill referral code + view from URL params on mount. Deriving initial
    // state from the URL once at mount requires a synchronous setState here.
    /* eslint-disable react-hooks/set-state-in-effect */
  ...
  useEffect(() => {
    // No configured client → nothing to bootstrap; stop the loading skeleton.
    ...
    async function bootstrap() {
      try {
        ...
        if (session) {
          ...
          // Block access until email is confirmed (only when the platform
          // requires it). Without this, middleware bounces the unconfirmed
          // user /dash → /auth and bootstrap() sends them right back → an
          // infinite redirect loop. Skip during the password-recovery flow.
          const isRecovery =
  ...
  useEffect(() => {
    ...
    // Reflect the username's validity synchronously as the user types; the async
    // uniqueness check runs after the debounce below.
    /* eslint-disable react-hooks/set-state-in-effect */
    ...
    <div className="w-full">
      ...
      {view === 'sign-in' && signInStep === 'email' && (
        <div className="animate-in fade-in slide-in-from-right-6 duration-300">
          ...
          <OAuthBlock handleOAuth={handleOAuth} loading={loading} />
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <PrimaryBtn label="Continue" loading={loading} />
          </form>
      ...
      {view === 'sign-in' && signInStep === 'password' && (
        <div className="animate-in fade-in slide-in-from-right-6 duration-300">
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <ErrBanner error={error} />
            <PrimaryBtn label="Sign in" loading={loading} />
          </form>
      ...
      {view === 'sign-in' && signInStep === 'otp' && (
        <div className="animate-in fade-in slide-in-from-right-6 duration-300">
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            />
            <ErrBanner error={error} />
            <PrimaryBtn label="Verify code" loading={loading} />
          </form>
      ...
      {view === 'sign-up' && accountType && signInStep === 'email' && (
        <div className="animate-in fade-in slide-in-from-right-6 duration-300">
          ...
          {registrationOpen && <OAuthBlock handleOAuth={handleOAuth} loading={loading} />}
      ...
      {view === 'sign-up' && signInStep === 'details' && (
        <div className="animate-in fade-in slide-in-from-right-6 duration-300">
          ...
          <form onSubmit={handleSubmit} className="space-y-2.5">
            ...
            />
            <ErrBanner error={error} />
            <div className="pt-1"><PrimaryBtn label="Create Account" loading={loading} /></div>
          </form>
      ...
      {view === 'forgot-password' && (
        <div className="animate-in fade-in slide-in-from-right-6 duration-300">
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            />
            <ErrBanner error={error} />
            <PrimaryBtn label="Send reset link" loading={loading} />
          </form>
          <div className="mt-4 text-center">
      ...
      {view === 'magic-link' && (
        <div className="animate-in fade-in slide-in-from-right-6 duration-300">
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            />
            <ErrBanner error={error} />
            <PrimaryBtn label="Send magic link" loading={loading} />
          </form>
          <div className="mt-4 text-center">
            <button type="button" onClick={() => switchView('sign-in')} className="text-[13px] text-[#0071e3] font-medium hover:underline inline-flex items-center gap-1">
              <ArrowLeft size={12} /> Back to sign in
      ...
      {view === 'update-password' && (
        <div className="animate-in fade-in slide-in-from-right-6 duration-300">
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <ErrBanner error={error} />
            <PrimaryBtn label="Update password" loading={loading} />
          </form>
        </div>
      )}
