...
async function logActivity(userId, type, content, metadata = {}) {
  try {
...
function CertificateModal({ cert, courseTitle, onClose }) {
  return (
...
export default function LessonViewer() {
  ...
  // Persist lessons/course in refs so keyboard handler always has fresh values
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const current = lessonsData.find((l) => l.id == targetId) || lessonsData[0];
      ...
      if (current && uid) {
        ...
        if (isMounted) {
          setIsCompleted(progressRes.data?.status === "completed");
          setCompletedLessons(new Set((allProgressRes.data ?? []).map((p) => p.lesson_id)));
  ...
  // ── Real-time subscription for lesson progress ──────────────────────────────
  useEffect(() => {
  ...
  // ── Sync isCompleted when lesson changes ────────────────────────────────────
  useEffect(() => {
    if (currentLesson) {
      setIsCompleted(completedLessons.has(currentLesson.id));
    }
  ...
  useEffect(() => {
    const handleKey = (e) => {
      ...
      if (e.key === "ArrowRight" && idx < ls.length - 1) {
        router.push(`/LearnPage/${courseId}?lessonId=${ls[idx + 1].id}`);
      }
  ...
  const handleMarkComplete = async () => {
    ...
    try {
      ...
      logActivity(userId, "lesson_completed", `Completed lesson: ${currentLesson.title}`, {
        course_id: courseId,
        lesson_id: currentLesson.id,
      });

      // Certificate only when ALL lessons in the course are done
      ...
      if (allDone) {
        ...
        if (!certError && newCert) {
          ...
          logActivity(
            userId,
            "certificate_earned",
            `Earned certificate for: ${course?.title ?? "course"}`,
            { course_id: courseId, cert_id: newCert.id }
          );
        }

        logActivity(userId, "course_completed", `Completed course: ${course?.title ?? "course"}`, {
          course_id: courseId,
        });
      } else {
        ...
        if (next) {
          setTimeout(() => router.push(`/LearnPage/${courseId}?lessonId=${next.id}`), 500);
        }
      }
    } catch (err) {
    ...
    <>
      ...
      <div className="min-h-screen bg-white dark:bg-black text-gray-900 dark:text-gray-100 pb-24">
        ...
        <nav className="sticky top-0 w-full border-b border-gray-200 dark:border-gray-800 bg-white/90 dark:bg-gray-900/90 backdrop-blur-xl z-50">
          <div className="max-w-5xl mx-auto px-4 sm:px-6 h-16 flex items-center gap-3">
            <button
              ...
              className="lg:hidden flex items-center justify-center w-9 h-9 rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors shrink-0"
            ...
            <button
              onClick={() => router.push("/LearnPage")}
              className="flex items-center gap-1.5 text-sm font-bold text-gray-600 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors shrink-0"
            >
              <ArrowLeft size={15} />
            ...
            <div className="flex-1 flex items-center gap-2 min-w-0">
              ...
              <span className="text-xs font-black text-gray-500 dark:text-gray-400 shrink-0 tabular-nums">
            ...
            {certificate && (
              <a
                ...
                className="flex items-center gap-1.5 text-xs font-bold text-amber-600 dark:text-amber-500 bg-amber-50 dark:bg-amber-900/20 px-3 py-1.5 rounded-full border border-amber-200 dark:border-amber-800/50 hover:border-amber-400 transition-colors shrink-0"
              >
                <Award size={13} />
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 grid grid-cols-1 lg:grid-cols-[260px_1fr] gap-6">
          ...
          <aside
            className={`
              fixed inset-y-0 left-0 z-50 w-72 bg-white dark:bg-gray-950 border-r border-gray-200 dark:border-gray-800 p-4 pt-5 overflow-y-auto transition-transform duration-300 shadow-2xl
              lg:static lg:w-auto lg:bg-transparent lg:border-0 lg:p-0 lg:shadow-none lg:block lg:translate-x-0
              ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}
            `}
          >
            {/* Mobile sidebar header */}
            <div className="flex items-center justify-between mb-4 lg:hidden">
              <span className="text-sm font-black text-gray-900 dark:text-gray-100 truncate pr-2">
                {course?.title ?? "Lessons"}
              </span>
              <button
                onClick={() => setSidebarOpen(false)}
                className="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 dark:hover:bg-gray-800 shrink-0"
              >
                <X size={16} />
              </button>
            </div>
            ...
            <div className="sticky top-24 bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-800 rounded-2xl p-3">
              <div className="hidden lg:block px-1 mb-3">
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-wider truncate mb-1">
                  {course?.title ?? "Lessons"}
                </p>
                <p className="text-[10px] text-gray-400 font-bold">
                  {completedLessons.size}/{lessons.length} complete · {progress}%
                </p>
              </div>
                ...
                {lessons.map((lesson, idx) => {
                  ...
                  return (
                    <button
                      key={lesson.id}
                      onClick={() => {
                        router.push(`/LearnPage/${courseId}?lessonId=${lesson.id}`);
                    ...
                    >
                      <div
                        className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-black shrink-0 transition-all ${
                          active
                      ...
                      {done && !active && (
                        <span className="ml-auto text-emerald-500 shrink-0 text-xs">✓</span>
                      )}
                    </button>
                  );
                })}
          ...
          <main>
            ...
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-black text-gray-900 dark:text-gray-100 tracking-tight leading-tight mb-8">
              {currentLesson.title}
            </h1>
            ...
            <div className="mt-12 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <p className="text-xs text-gray-400 dark:text-gray-500 font-medium">
                {isCompleted ? (
                  <span className="text-emerald-600 dark:text-emerald-400 font-bold flex items-center gap-1">
                    <CheckCircle2 size={13} /> Progress saved to your account
                  </span>
                ) : (
              ...
              <button
                ...
                className={`flex items-center gap-2 px-6 py-3 font-bold rounded-xl transition-all shadow-sm shrink-0 ${
                  isCompleted
            ...
            <div className="flex items-center justify-between mt-12 pt-8 border-t border-gray-200 dark:border-gray-800 gap-4">
              {prevLesson ? (
                <button
                  onClick={() =>
                    router.push(`/LearnPage/${courseId}?lessonId=${prevLesson.id}`)
                  }
                  className="group flex flex-col items-start gap-1 text-left min-w-0 max-w-[48%]"
                >
                  <span className="flex items-center gap-1 text-xs font-bold text-gray-400 uppercase tracking-wider group-hover:text-blue-500 transition-colors">
                    <ArrowLeft size={13} /> Previous
                  </span>
                  <span className="text-sm font-semibold text-gray-700 dark:text-gray-300 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors truncate w-full">
                    {prevLesson.title}
                  </span>
                </button>
              ) : (
              ...
              {nextLesson ? (
                <button
                  ...
                  className="group flex flex-col items-end gap-1 text-right min-w-0 max-w-[48%]"
                >
                  <span className="flex items-center gap-1 text-xs font-bold text-gray-400 uppercase tracking-wider group-hover:text-blue-500 transition-colors">
            ...
            <p className="text-center text-[10px] text-gray-300 dark:text-gray-700 mt-6 select-none">
