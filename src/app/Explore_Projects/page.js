...
export default function ExploreProjects() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900 selection:bg-blue-500/30 overflow-x-hidden relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredProjects.length > 0 ? (
            filteredProjects.map((proj, i) => (
              <div key={i} onClick={() => setSelectedProject(proj)} className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-[2rem] p-6 hover:border-blue-500/50 dark:hover:border-blue-500/50 hover:shadow-lg transition-all group cursor-pointer flex flex-col h-full">
              ...
              <p className="text-gray-600 dark:text-gray-300 text-sm leading-relaxed mb-6 flex-1">{proj.desc}</p>
              <div className="flex items-center justify-between pt-4 border-t border-gray-100 dark:border-gray-800">
                <div className="flex items-center gap-4 text-sm font-bold text-gray-500 dark:text-gray-400">
                  <span className="flex items-center gap-1.5 hover:text-amber-500 dark:hover:text-amber-400 transition-colors"><Star size={16} /> {proj.stars}</span>
                  <span className="flex items-center gap-1.5 hover:text-blue-500 dark:hover:text-blue-400 transition-colors"><GitBranch size={16} /> {proj.forks}</span>
                </div>
      ...
      <footer className="border-t border-gray-200 dark:border-gray-800 py-10 text-center text-gray-500 dark:text-gray-400 text-xs font-mono uppercase tracking-widest relative z-10 bg-white dark:bg-gray-900">
            ...
            <div className="p-6 sm:p-8 border-b border-gray-100 dark:border-gray-800 flex items-start justify-between bg-gray-50/50 dark:bg-gray-800/50">
              <div className="flex items-center gap-4">
                 ...
                 <div>
                   ...
                   <p className="text-sm font-bold text-gray-500 dark:text-gray-400">Maintained by <span className="text-blue-600 dark:text-blue-400 cursor-pointer hover:underline">{selectedProject.author}</span></p>
                 </div>
              </div>
