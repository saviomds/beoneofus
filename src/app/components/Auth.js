...
export default function AuthForm() {
      ...
      switch (view) {
        ...
        default: {
          if (signInStep === 'email') {
            ...
            // Step 3: verify OTP code then sign in
  ...
  // Shared Apple-style input class
  const iCls =
  ...
  const PwField = ({ id, value, onChange, placeholder, autoFocus: af }) => (
    <div className="relative">
      <input id={id} type={showPw ? 'text' : 'password'} required value={value} onChange={onChange}
        autoFocus={af} placeholder={placeholder} autoComplete={id === 'confirm-password' ? 'new-password' : 'current-password'}
    ...
    <div className="w-full">
      ...
      {view === 'sign-in' && signInStep === 'password' && (
        <div className="animate-in fade-in slide-in-from-right-6 duration-300">
          <div className="text-center mb-7">
            <h1 className="text-2xl font-semibold tracking-tight text-gray-900 dark:text-white">Sign in</h1>
            <button type="button" onClick={() => { setSignInStep('email'); setPassword(''); setError(null); }}
          ...
          <form onSubmit={handleAuth} className="space-y-3">
            <PwField id="auth-password" value={password} onChange={(e) => setPassword(e.target.value)}
          ...
          <div className="mt-5 space-y-2 text-center">
            <button type="button" onClick={() => switchView('forgot-password')}
              className="text-[13px] text-[#0071e3] font-medium hover:underline block mx-auto">
              Forgot password?
            </button>
            <button type="button" onClick={() => switchView('magic-link')}
              className="text-[12px] text-gray-400 dark:text-gray-500 hover:text-gray-500 transition-colors block mx-auto">
      ...
      {view === 'sign-in' && signInStep === 'otp' && (
        <div className="animate-in fade-in slide-in-from-right-6 duration-300">
          ...
          <form onSubmit={handleAuth} className="space-y-3">
            <input id="otp-code" type="text" inputMode="numeric" maxLength={6} required autoFocus
          ...
          <div className="mt-5 flex flex-col items-center gap-2">
            ...
            <button type="button" onClick={() => { setSignInStep('email'); setOtpCode(''); setError(null); setResendCooldown(0); }}
      ...
      {view === 'sign-up' && signInStep === 'details' && (
        <div className="animate-in fade-in slide-in-from-right-6 duration-300">
          <div className="text-center mb-7">
            <h1 className="text-2xl font-semibold tracking-tight text-gray-900 dark:text-white">Create Account</h1>
            <button type="button" onClick={() => { setSignInStep('email'); setPassword(''); setConfirmPassword(''); setUsername(''); setError(null); }}
              className="mt-1.5 inline-flex items-center gap-1.5 text-[13px] text-[#0071e3] hover:underline transition-colors">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/>
                <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/>
          ...
          <form onSubmit={handleAuth} className="space-y-2.5">
            <div className="relative">
              <input id="auth-username" type="text" required autoFocus value={username}
                onChange={(e) => setUsername(e.target.value.toLowerCase())}
                className={`${iCls} pr-10 ${
                  usernameStatus === 'taken' || usernameStatus === 'invalid' ? 'ring-2 ring-red-400/40' :
                  usernameStatus === 'available' ? 'ring-2 ring-emerald-400/40' : ''
                }`}
                placeholder="Username" autoComplete="username" maxLength={20} />
            ...
            {usernameStatus === 'invalid' && username && <p className="text-xs text-red-500 px-1">3–20 chars: lowercase, numbers, _ or -</p>}
            {usernameStatus === 'taken' && <p className="text-xs text-red-500 px-1">Username already taken</p>}
            <div>
      ...
      {view === 'magic-link' && (
        <div className="animate-in fade-in slide-in-from-right-6 duration-300">
          <div className="mb-7 text-center">
            ...
            <p className="text-[13px] text-gray-500 dark:text-gray-400 mt-1.5 max-w-[260px] mx-auto leading-relaxed">
              Get a secure link in your inbox — no password needed.
            </p>
          </div>
          <form onSubmit={handleAuth} className="space-y-3">
            <input id="auth-email" type="email" required autoFocus value={email}
      ...
      {view === 'update-password' && (
        <div className="animate-in fade-in slide-in-from-right-6 duration-300">
          ...
          <form onSubmit={handleAuth} className="space-y-3">
            ...
            <PwField id="confirm-password" value={confirmPassword}
