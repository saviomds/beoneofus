...
export async function POST(req) {
  try {
    const { orderId, userId, email, priceUsd } = await req.json();

    if (!orderId || !userId || !email || priceUsd === undefined) {
    ...
    const reference = `bou_svc_${orderId.slice(0, 8)}_${userId.slice(0, 8)}_${Date.now()}`;
