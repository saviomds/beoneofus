...
const markdownComponents = {
  ...
  code({ node, inline, className, children, ...props }) {
    ...
    return !inline && match ? (
      <div className="rounded-xl overflow-hidden my-4 border border-gray-200 shadow-sm bg-[#1E1E1E]">
      ...
      <code {...props} className="bg-gray-100 text-gray-800 px-1.5 py-0.5 rounded-md font-mono text-[12px] border border-gray-200">
        {children}
      </code>
    );
  }
...
export default function AiAssistantContent() {
  ...
  return (
    <div className="w-full flex flex-col h-[calc(100vh-120px)] bg-white border border-gray-200 rounded-[2rem] shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-700">
      {/* Header */}
      <div className="flex items-center gap-3 p-6 border-b border-gray-100 bg-gray-50/50">
        <div className="w-10 h-10 rounded-xl bg-gray-900 text-white flex items-center justify-center shrink-0 shadow-sm">
          <Bot size={20} />
        </div>
        <div>
          <h2 className="text-lg font-bold text-gray-900">beoneofus AI</h2>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-gray-50/30">
        ...
        {messages.map((msg, index) => (
          <div key={index} className={`flex gap-4 ${msg.role === "user" ? "flex-row-reverse" : "flex-row"}`}>
            <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${msg.role === "user" ? "bg-blue-100 text-blue-600" : "bg-gray-900 text-white"}`}>
              {msg.role === "user" ? <User size={16} /> : <Bot size={16} />}
            </div>
            <div className={`max-w-[85%] sm:max-w-[75%] rounded-2xl p-4 text-sm ${msg.role === "user" ? "bg-blue-600 text-white shadow-md shadow-blue-500/20 whitespace-pre-wrap" : "bg-white border border-gray-200 text-gray-800 shadow-sm"}`}>
              {msg.role === "user" ? (
                msg.content
              ) : msg.isNew ? (
        ...
        {isLoading && (
          <div className="flex gap-4 flex-row">
            <div className="w-8 h-8 rounded-full flex items-center justify-center shrink-0 bg-gray-900 text-white">
              <Bot size={16} />
            </div>
            <div className="max-w-[80%] rounded-2xl p-4 bg-white border border-gray-200 text-gray-800 shadow-sm flex items-center gap-2">
              <Loader2 size={16} className="animate-spin text-gray-900" /> <span className="text-xs text-gray-500 font-medium">Thinking...</span>
            </div>
          </div>
        )}
      ...
      <div className="p-4 bg-white border-t border-gray-100">
        <form onSubmit={sendMessage} className="flex items-center gap-3 max-w-4xl mx-auto">
          <input
            ...
            className="flex-1 bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-xl px-5 py-3.5 focus:outline-none focus:border-blue-500/50 focus:ring-4 focus:ring-blue-500/10 transition-all"
            disabled={isLoading || isFetchingHistory}
          />
          <button
            type="submit"
            disabled={!input.trim() || isLoading || isFetchingHistory}
            className="bg-gray-900 hover:bg-gray-800 disabled:opacity-50 disabled:hover:bg-gray-900 text-white rounded-xl p-3.5 transition-all shadow-sm flex-shrink-0"
          >
