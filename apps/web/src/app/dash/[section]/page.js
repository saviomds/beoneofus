...
import { useDashboard } from '../content/DashboardContext';
...
export default function DashSection() {
  ...
  // Shared dashboard session — same source the sidebar uses, so the feed
  // composer never disagrees with the rest of the shell.
  const { session, sessionReady } = useDashboard();

  // Redirect unknown / removed sections to home instead of calling notFound()
  // in a client component (which can cause hydration issues).
  useEffect(() => {
      ...
      {section === 'feed' && (
        <div className="w-full animate-in fade-in slide-in-from-bottom-2 duration-400 mb-4">
          ...
          {session ? (
            ...
            <div className="w-full h-28 bg-gray-100 dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 animate-pulse mb-4" />
          ) : (
