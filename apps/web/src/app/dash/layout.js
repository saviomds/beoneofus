...
import { nextAuthState } from './authSession.mjs';
...
function BottomNav({ pathname }) {
  ...
  // Hide on messages — the chat composer sits at the bottom and the nav would cover it
  if (section === 'messages') return null;
...
function PickUsernameModal({ onDone }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      if (error?.name === 'AbortError') return; // request was cancelled, do nothing.
  ...
  const handleSave = async () => {
    ...
    // Re-check availability right before saving to minimize race condition window.
...
export default function DashLayout({ children }) {
  ...
  useEffect(() => {
    let active = true;
    // Every transition — the initial getSession() and every later auth event —
    // goes through the same reducer (src/app/dash/authSession.mjs) so the
    // optimistic `authenticated` seeded above can still be corrected, and the
    // "session expired" card only fires on a genuine mid-session drop.
    const apply = (event, session) => {
      if (!active) return;
      dbg('auth', event, session ? `uid=${session.user.id}` : '(no session)');
      setAuthState(prev => nextAuthState(prev, event, session));
    };

    supabase.auth.getSession().then(({ data: { session } }) => apply('GET_SESSION', session));
  ...
  return (
    <DashboardProvider>
      ...
      <DashLayoutContent isAuthenticated={authState === 'authenticated'}>{children}</DashLayoutContent>
    </DashboardProvider>
  );
}
