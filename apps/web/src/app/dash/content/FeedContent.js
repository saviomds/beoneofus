...
import {
  ...
  GitBranch, Link as LinkIcon, ExternalLink, Sparkles, Loader2,
...
export default function FeedContent() {
  const { t } = useLanguage();
  // Auth comes from the shared dashboard session (same source as the sidebar),
  // NOT a local one-shot getSession() — that race was why the feed treated a
  ...
  const fetchPosts = useCallback(async (silent = false) => {
    ...
    const isAuthError = (e) => {
      ...
      return /refresh token|invalid refresh|jwt|unauthorized|not authenticated|auth/i.test(message);
    };
    ...
    if (isAuthError(lastError)) {
      setPosts([]);
      setFetchError(t('feed.error_generic') || 'Unable to load feed right now. Please refresh or sign in again.');
    } else {
      setFetchError(
  ...
  useEffect(() => {
    ...
    const initFeed = async () => {
      try {
        ...
        if (isMounted) {
          setFetchError(error?.message || 'Unable to load feed');
        }
      }
    };
  ...
  return (
    ...
    >
      {loading && posts.length === 0 && !fetchError && (
        <div className="rounded-2xl border border-blue-200/70 dark:border-blue-800/50 bg-blue-50/70 dark:bg-blue-900/20 px-4 py-3 text-sm font-medium text-blue-700 dark:text-blue-300">
          Preparing your feed…
        </div>
      )}
