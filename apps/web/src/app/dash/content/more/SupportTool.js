...
const SupportTool = () => {
  ...
  const handleSubmit = async (text) => {
    ...
    // Support is handled by the team over email — open the user's mail client
    // with the issue pre-filled and acknowledge in the thread.
    const subject = encodeURIComponent(`[${category}] Support request`);
    const body = encodeURIComponent(cur);
    try {
      window.location.href = `mailto:support@beoneofus.work?subject=${subject}&body=${body}`;
    } catch { /* no mail client */ }
    setMessages(prev => [...prev, {
      id: Date.now(),
      role: "ai",
      content: "Thanks — your message has been prepared for our support team at **support@beoneofus.work**. If your mail client didn't open, email us directly and we'll reply within 24 hours. You may also find an instant answer in the FAQ below.",
      isNew: false,
    }]);
      ...
      <div className="relative overflow-hidden rounded-2xl border border-blue-200 dark:border-blue-900/40 bg-gradient-to-br from-blue-600 to-blue-700 dark:from-blue-700 dark:to-blue-900 p-5 text-white shadow-lg">
        ...
        <div className="relative flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-blue-100/80 leading-relaxed">Tell us about your issue — account help, technical issues, billing, and more.</p>
          </div>
          <div className="shrink-0 text-right hidden sm:block">
            <p className="text-[10px] text-blue-200/60 uppercase tracking-widest">Avg. response</p>
            <p className="text-sm font-black text-white">~24 hrs</p>
          </div>
        </div>
      </div>
